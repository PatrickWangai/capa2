...
export default function AppLayout() {
  ...
  const sidebarBorder  = 'rgba(0,0,0,0.08)';
  const navInactive    = 'var(--nav-text)';
  const drawerBg       = '#ffffff';
  const drawerBorder   = 'rgba(0,0,0,0.08)';
  const drawerHeadText = '#131722';
  const drawerLabelClr = '#9fa3b0';
  const drawerCloseClr = '#787b86';
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'var(--bg-base)' }}>
        ...
        <aside style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: collapsed ? 'center' : 'space-between', padding: collapsed ? '16px 0' : '16px 12px 16px 20px', borderBottom: `1px solid ${sidebarBorder}`, flexShrink: 0 }}>
            ...
            <button
              ...
              style={{ color: '#b2b5be', background: 'none', border: 'none', cursor: 'pointer', padding: 6, borderRadius: 8, display: 'flex', flexShrink: 0, transition: 'color 0.15s' }}
              onMouseEnter={e => (e.currentTarget.style.color = '#131722')}
              onMouseLeave={e => (e.currentTarget.style.color = '#b2b5be')}>
          ...
          {!collapsed && (
            <div style={{ padding: '10px 12px 4px' }}>
              <button onClick={() => setSearch(true)}
                style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', borderRadius: 10, background: '#f3f5fb', border: '1px solid rgba(0,0,0,0.08)', color: '#9fa3b0', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, textAlign: 'left', transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#eaecf4'; e.currentTarget.style.color = '#4a4f5e'; }}
                onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#f3f5fb'; e.currentTarget.style.color = '#9fa3b0'; }}>
                <Search size={15} /><span style={{ flex: 1 }}>Search stocks…</span>
                <kbd style={{ fontSize: 10, padding: '2px 5px', borderRadius: 4, background: 'rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.08)', color: '#b2b5be' }}>⌘K</kbd>
              </button>
            </div>
          )}
          ...
          {collapsed && (
            <div style={{ padding: '10px 0 4px', display: 'flex', justifyContent: 'center' }}>
              <button onClick={() => setSearch(true)} title="Search stocks"
                style={{ color: '#9fa3b0', background: '#f3f5fb', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 10, cursor: 'pointer', padding: 8, display: 'flex', transition: 'all 0.15s' }}
                onMouseEnter={e => { e.currentTarget.style.backgroundColor = '#eaecf4'; e.currentTarget.style.color = '#4a4f5e'; }}
                onMouseLeave={e => { e.currentTarget.style.backgroundColor = '#f3f5fb'; e.currentTarget.style.color = '#9fa3b0'; }}>
                <Search size={16} />
              </button>
          ...
          <div style={{ padding: collapsed ? '12px 0' : 12, borderTop: `1px solid ${sidebarBorder}`, flexShrink: 0 }}>
            {collapsed ? (
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
                ...
                <button onClick={handleLogout} title="Sign out" style={{ color: '#b2b5be', background: 'none', border: 'none', cursor: 'pointer', padding: 4, borderRadius: 6, display: 'flex' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 12, backgroundColor: '#f3f5fb' }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 13, fontWeight: 600, color: '#131722', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.firstName} {user?.lastName}</p>
                  <p style={{ fontSize: 11, color: '#787b86', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.email}</p>
                </div>
                <button onClick={handleLogout} style={{ color: '#b2b5be', background: 'none', border: 'none', cursor: 'pointer', padding: 4, borderRadius: 6, display: 'flex' }}
                  onMouseEnter={e => (e.currentTarget.style.color = '#f23645')}
                  onMouseLeave={e => (e.currentTarget.style.color = '#b2b5be')}>
                  <LogOut size={15} />
                </button>
        ...
        <aside style={{ position: 'fixed', top: 0, left: 0, width: 256, height: '100vh', zIndex: 50, display: 'flex', flexDirection: 'column', backgroundColor: 'var(--sidebar-bg)', backdropFilter: 'saturate(160%) blur(28px)', WebkitBackdropFilter: 'saturate(160%) blur(28px)', borderRight: `1px solid ${sidebarBorder}`, boxShadow: '4px 0 24px rgba(0,0,0,0.4)', overflowY: 'auto', transform: mobileOpen ? 'translateX(0)' : 'translateX(-100%)', transition: 'transform 0.25s cubic-bezier(0.4,0,0.2,1)' }}>
          ...
          <div style={{ padding: '10px 12px 4px' }}>
            <button onClick={() => { setSearch(true); setMobileOpen(false); }}
              style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%', padding: '8px 12px', borderRadius: 10, background: '#f3f5fb', border: '1px solid rgba(0,0,0,0.08)', color: '#9fa3b0', cursor: 'pointer', fontFamily: 'inherit', fontSize: 14, textAlign: 'left' }}>
              <Search size={15} /><span style={{ flex: 1 }}>Search stocks…</span>
            </button>
          </div>
          ...
          <div style={{ padding: 12, borderTop: `1px solid rgba(0,0,0,0.08)` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 12, backgroundColor: '#f3f5fb' }}>
              <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'linear-gradient(135deg,var(--accent),var(--accent-dark))', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: 14, fontWeight: 600, flexShrink: 0 }}>
                {user?.firstName?.[0]}{user?.lastName?.[0]}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 13, fontWeight: 600, color: '#131722', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user?.firstName} {user?.lastName}</p>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        ...
        {isMobile && (
          <header style={{ height: 52, flexShrink: 0, display: 'flex', alignItems: 'center', padding: '0 16px', gap: 12, backgroundColor: '#ffffff', borderBottom: `1px solid rgba(0,0,0,0.08)` }}>
            <button onClick={() => setMobileOpen(true)} style={{ color: '#787b86', background: 'none', border: 'none', cursor: 'pointer', padding: 6, display: 'flex', borderRadius: 8 }}>
              <Menu size={22} />
            </button>
            <div style={{ flex: 1, display: 'flex', justifyContent: 'center' }}>
              <CapaLogo size={34} />
            </div>
            <button onClick={() => setSearch(true)} style={{ color: '#787b86', background: 'none', border: 'none', cursor: 'pointer', padding: 6, display: 'flex', borderRadius: 8 }}>
              <Search size={20} />
            </button>
          </header>
        )}

        <main style={{ flex: 1, overflowY: 'auto', padding: isMobile ? '16px 12px' : '24px', background: 'var(--bg-base)' }}>
          <Outlet />
        </main>
      </div>
    </div>
