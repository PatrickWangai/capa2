...
function usePreloader() {
  ...
  // Sized to the actual viewport diagonal (not a fixed guess) so the hole's
...
function Preloader() {
  ...
  return (
    <div style={{
      ...
      display: 'flex', alignItems: 'center', justifyContent: 'center',
      // Reveal grows a hole shaped like the C mark — using a *filled*
      // silhouette (capa-c-icon-filled.png), not the actual badge asset,
      // which is a ring with a hollow middle and reads as "a ring" when
      // used directly as a mask. The filled version was generated by
      // morphologically closing the ring's donut hole and its small side
      // notch, then flood-filling the enclosed interior solid, so the
      // hole's outer silhouette still matches the real badge shape.
      WebkitMaskImage: 'linear-gradient(#000,#000), url(/capa-c-icon-filled.png)',
      maskImage: 'linear-gradient(#000,#000), url(/capa-c-icon-filled.png)',
