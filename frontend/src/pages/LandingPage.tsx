...
export default function LandingPage() {
  return (
    <div style={{ backgroundColor: '#000000', color: '#ffffff', fontFamily: '-apple-system,BlinkMacSystemFont,"SF Pro Display","Helvetica Neue",Arial,sans-serif', WebkitFontSmoothing: 'antialiased' }}>
      ...
      <nav style={{ position: 'sticky', top: 0, zIndex: 100, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', backgroundColor: 'rgba(0,0,0,0.72)', backdropFilter: 'saturate(180%) blur(20px)', WebkitBackdropFilter: 'saturate(180%) blur(20px)', borderBottom: '1px solid rgba(84,84,88,0.45)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <OrangeIcon size={26} />
          <span style={{ fontWeight: 600, fontSize: 17, letterSpacing: '-0.02em', color: '#ffffff' }}>Capa</span>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Link to="/login" style={{ padding: '5px 14px', borderRadius: 980, fontSize: 13, fontWeight: 500, color: '#ffffff', textDecoration: 'none', backgroundColor: 'rgba(255,255,255,0.08)' }}>Log In</Link>
      ...
      <section style={{ textAlign: 'center', padding: '100px 24px 80px', maxWidth: 980, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 'clamp(48px, 7vw, 96px)', fontWeight: 700, letterSpacing: '-0.03em', lineHeight: 1.04, color: '#ffffff', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <Link to="/login" style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '12px 24px', borderRadius: 980, backgroundColor: 'rgba(255,255,255,0.08)', color: '#ffffff', textDecoration: 'none', fontSize: 17, fontWeight: 500 }}>
            Sign In
          </Link>
        </div>
      </section>
      ...
      <section style={{ padding: '80px 24px', maxWidth: 980, margin: '0 auto' }}>
        ...
        <h2 style={{ fontSize: 'clamp(32px,5vw,56px)', fontWeight: 700, letterSpacing: '-0.03em', textAlign: 'center', color: '#ffffff', marginBottom: 56, lineHeight: 1.08 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
          {features.map(({ icon: Icon, title, desc }) => (
            <div key={title} style={{ backgroundColor: '#1c1c1e', borderRadius: 20, padding: 32, boxShadow: '0 0 0 0.5px rgba(255,255,255,0.08)' }}>
              ...
              <h3 style={{ fontSize: 19, fontWeight: 600, color: '#ffffff', marginBottom: 8, letterSpacing: '-0.02em' }}>{title}</h3>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ backgroundColor: '#000000', borderTop: '1px solid rgba(0,0,0,0.1)', padding: '32px 24px' }}>
        <div style={{ maxWidth: 980, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <OrangeIcon size={20} />
            <span style={{ fontSize: 13, fontWeight: 600, color: '#ffffff' }}>Capa</span>
          </div>
