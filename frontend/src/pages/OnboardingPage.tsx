      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {RISK_OPTIONS.map(o => (
                <button key={o.value} onClick={() => setAnswers(a => ({ ...a, risk: o.value }))}
                  style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '16px 20px', borderRadius: 14, border: `1.5px solid ${answers.risk === o.value ? ACCENT : 'rgba(84,84,88,0.5)'}`, backgroundColor: answers.risk === o.value ? 'rgba(245,130,31,0.08)' : '#1c1c1e', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.15s' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {GOAL_OPTIONS.map(o => (
                <button key={o.value} onClick={() => setAnswers(a => ({ ...a, goal: o.value }))}
                  style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', gap: 8, padding: '18px 16px', borderRadius: 14, border: `1.5px solid ${answers.goal === o.value ? ACCENT : 'rgba(84,84,88,0.5)'}`, backgroundColor: answers.goal === o.value ? 'rgba(245,130,31,0.08)' : '#1c1c1e', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.15s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {HORIZON_OPTIONS.map(o => (
                <button key={o.value} onClick={() => setAnswers(a => ({ ...a, horizon: o.value }))}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', borderRadius: 14, border: `1.5px solid ${answers.horizon === o.value ? ACCENT : 'rgba(84,84,88,0.5)'}`, backgroundColor: answers.horizon === o.value ? 'rgba(245,130,31,0.08)' : '#1c1c1e', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', transition: 'all 0.15s' }}>
        ...
        <div style={{ marginTop: 28, display: 'flex', gap: 12 }}>
          ...
          <button onClick={next} disabled={!canNext()}
            style={{ flex: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '14px', borderRadius: 12, backgroundColor: canNext() ? ACCENT : 'rgba(245,130,31,0.3)', color: '#fff', border: 'none', fontSize: 16, fontWeight: 500, cursor: canNext() ? 'pointer' : 'not-allowed', fontFamily: 'inherit', transition: 'background 0.15s' }}>
